...
- **Ready**: Cell lines ready for database ingestion (finalized, awaiting ingestion)
- **Registered**: Cell lines successfully ingested into external database (final status)
...
5. Database ingests v2, updates log
...
**Location:** `services/backend/data/ingestion_log.csv` (shared directory accessible by both systems)

**Management:**
- Written by database admin system after ingestion attempts
